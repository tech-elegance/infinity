...
const AddProject = ({ company, types }) => {
  ...
  const onSubmit = (data) => {
    if (data.check == "home") {
      ...
      axios
        .post(
          `${process.env.BACK_END_URL}/company/${company._id}/type/${types._id}/organization`,
          obj
        )
        .then((res) => {
          console.log(res);
  ...
  return (
    <div>
      <Head>
        <title>เพิ่มโครงการ - Admin</title>
          ...
          <div className="w-full lg:mb-8 mt-4 lg:mx-4 pb-4 bg-white lg:rounded-2xl">
            <div className="p-8">
              <h1 className="py-4 flex">
                ...
                <span className="ml-2">เพิ่มโครงการ</span>
              </h1>
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="mt-12 ">
                  <button
                    type="submit"
                    className="bg-green-500 hover:bg-green-700 px-4 py-2 rounded-md text-white float-right font-bold"
                  >
                    ยืนยัน
                  </button>
                </div>
