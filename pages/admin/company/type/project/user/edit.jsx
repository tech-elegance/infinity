...
const EditUser = ({ project }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (password != confirmPassword) {
      ...
      axios
        .put(
          `${process.env.BACK_END_URL}/organization/${project._id}/user/${id}`,
          obj
        )
        .then((response) => {
          // console.log(response);
          router.push("/admin/company/type/project/user?alert=UPDATE");
        })
        .catch((err) => {
  ...
  const onDelete = () => {
    if (checkname == name) {
      ...
      axios
        ...
        .then((res) => {
          router.push("/admin/company/type/project/user?alert=DELETE");
        })
        .catch((res) => {
          addToast("การเพิ่มข้อมูลมีปัญหา", {
            appearance: "error",
  ...
  // const developer = () => {
  //   return (
  //     <div className="mt-5 lg:grid-cols-2 gap-4 items-center">
  //       <p className={`text-md`}>
  ...
  return (
    <div>
      <Head>
        <title>แก้ไขผู้ใช้ภายในโครงการ - Admin</title>
          ...
          <div className="w-full lg:mb-8 mt-4 lg:mx-4 pb-4 bg-white lg:rounded-2xl">
            <div className="p-8">
              <h1 className="py-4 flex">
                ...
                <span className="ml-2">แก้ไขผู้ใช้ภายในโครงการ</span>
              </h1>
