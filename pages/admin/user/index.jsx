...
const User = ({ project }) => {
  ...
  try {
    var { data, error } = useSWR(`${process.env.BACK_END_URL}/users`, fetcher);
  } catch (err) {
    console.log(err);
  }
  console.log(data);
                ...
                <div className="grid grid-cols-2 gap-4 font-medium">
                  <div>
                    <Input
                      ...
                      label="ชื่อ นามสกุล/รายละเอียด/โครงการ"
                      placeholder="ค้นหา"
                      onChange={(e) => setSearched(e.target.value)}
                    />
                  </div>
                  <div>
                    <p className="text-center text-sm">บทบาท</p>
                    <div
                          ...
                          .filter(
                            (filt) =>
                              ...
                              filt.email
                                .toLowerCase()
                                .includes(searched.toLowerCase()) ||
                              JSON.stringify(filt.organization)
                                .toLowerCase()
                                .includes(searched.toLowerCase())
                          )
                          .filter((filt) =>
                              ...
                              <tr key={row._id} className="text-center ">
                                ...
                                <td className="px-3 py-3">
                                  {row.organization.map(
                                    (row2, no2) =>
                                      row2.organizationtype &&
                                      row2.organizationtype.company.title +
                                        " " +
                                        row2.title &&
                                      row2.title + ", "
                                  )}
                                ...
                                <td className="px-3 py-3 grid justify-items-center">
                                  <Link
                                    href={`/admin/user/edit?id=${row._id}&name=${row.name}&email=${row.email}&role=${row.role}&organization=${row.organization}`}
                                  >
